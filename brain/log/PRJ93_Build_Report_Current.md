...
| Track | Result |
|---|---|
| **Track A** (`brain/`, Python) | A0–A14 + A13 change-point + point deviation + briefing capstone + live-ingest/freshness + promote-and-serve + agent-evaluation framework (incl. scaled injection grid + labelling protocol). **207 pytest passing** (23 files). Full pipeline runs end-to-end via `scripts/run_all_venues.sh`. |
...
| **Reviews** | Scaled eval: code-reviewer **no HIGH** (1 MEDIUM — clean-baseline used the full stream not the fold-truncated one, understating precision for non-final folds — **fixed** (precision 0.68→0.87, recall/sensitivity unchanged); 1 LOW near-threshold semantics **fixed**); security-reviewer **no findings**. Agent-eval framework: code-reviewer **no HIGH/MEDIUM** (3 LOW — 2 dead config constants + an F1-NaN edge — **all fixed**); security-reviewer **no findings** (SQL parameterised, live judge triple-gated + `anthropic` not installed, offline seam can't fabricate a score, read-only except `eval_labels`). Promote-and-serve (v2.1): code-reviewer **no HIGH/MEDIUM** (2 LOW, both intended behaviour, no fix); security-reviewer **no findings**. Live ingest: code-reviewer 3 LOW (all inert-today) **fixed** (incl. a flaky read↔write conn race, hardened); security-reviewer **no findings**; doc-reviewer stale docs **fixed**. Briefing capstone: code 1 HIGH + 2 LOW **all fixed**; security clean. Point deviation: code clean; security 1 LOW **fixed**. Stock: security clean; code 1 MEDIUM + 1 LOW **fixed**. |
...
CSV / XLSX sources
  ...
  A6 forecasts ───────────────────────┴─► A12 stock cover ─► stock_cover(14) ─► (A6 re-run enriches its report)
                                                              │
  A10 FastAPI: /health /forecast /deviation/{check,scan,changepoint} /sop-gaps /checklist/discipline /stock/cover /briefing /freshness /refresh
                                                              │
  Track B (NestJS): BrainClient ─► BrainService ─► BrainProvider ─(IntegrationRegistry)─► AI-GM agent
                    8 tools: forecast_sales, check_deviation, find_sop_gaps, check_checklist, check_stock_cover, check_change_point, daily_briefing, data_freshness
                    Web: ForecastBandCard, DeviationCard, SopGapsCard, ChecklistCard, StockCoverCard, ChangePointCard, BriefingCard, FreshnessCard
```
...
| **Live ingest / freshness** | `ingest/sources/`, `ingest/live.py`, `ingest/refresh.py` | PASS (G0–G9) — three-tier model: T1 live facts (cached, inert while off), T2 `refresh()` appends closed days + advances `data_watermark` + auto-exog + rebuilds features (idempotent), T3 ladder re-fit only on weekly cadence or a confirmed change-point (a transaction never triggers it), logging `ladder_selection`; `/freshness` + `/refresh` (endpoints 11); freshness block on envelopes; 8th read-only Track-B tool; inert by default (csv) |
...
| brain tests | `pytest` | **207 passed** (23 files; +9 A13, +14 point deviation, +11 briefing, +18 live-ingest, +14 promote-and-serve, +22 agent-eval, +14 scaled-eval) |
