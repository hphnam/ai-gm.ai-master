...
| | | *(inventory row above is the **pre-8C-5** state and is kept as history.)* **Recomposed 2026-08-09:** heading renamed to **Contributions** per §3.5 (N1, N6); **five** contributions per U1; `sec:conclusion-objectives` (6.1) added before it per U6; the section measures **962**. C2 and RQ2's limb were both absent from the four-claim version. | | | |
...
| **6** | **Conclusions** | | **1,100** |
| 6.1 | Objectives revisited | **The three Student deliverables of `docs/PRJ93.md`**, one subsection each: committed, delivered, evidence, and any unmet part named in the same sentence as the delivered part. Cause cross-referenced to 5.5 (HC59), not re-argued. See §7 **U6**. | 400 |
...
**Conclusions, whole chapter — 1,100 budgeted against 2,328 measured. Added 2026-08-09 by
...
| **U6** | A2 / §2.1, the purpose cell for Conclusions **6.1** | "Each objective, achieved or not, stated plainly" | **The three Student deliverables of `docs/PRJ93.md`**, one subsection each | Phuong, explicit ruling, 2026-08-09, on the blocker raised at the 8C-5 discovery | **The cell presupposed a list the project does not have.** `docs/PRJ93.md` was read end to end (38 lines) on 2026-08-09: *"objective"* occurs **0** times, *"aim"* 0, *"goal"* 0, *"outcome"* 0, *"success criteri"* 0, *"deliverable"* **1**. It is the host's placement advertisement and offers three candidate referents — one research question, three brain components, three Student deliverables. The deliverables win because they are the only **numbered commitments** and are what an examiner comparing specification to dissertation will line up; the research question is one item and too coarse to structure 400 words, and the components describe an architecture rather than a commitment anyone made. **Two conditions attach — see below.** |

**Conditions on U6, binding on whoever composes 6.1.**

1. **An unmet part is named in the same sentence as the delivered part**, never in a later hedge. Deliverable (i) is met for the rhythm and deviation components and **unmeasured** for the agent; (ii) is met quantitatively and **unmet** qualitatively; (iii) is met. The graded-strength convention of `06_research_questions.md` §6 governs here exactly as it governs 1.4 and 5.4.
2. **6.1 names the cause and does not re-argue it.** The unmet parts are unmet because externally specified inputs never arrived, which 5.5 establishes under HC59; 6.1 cross-references that section. A conclusion reporting a shortfall without its cause reads as a failure to deliver, and one reporting it against the declared scope divergence reads as a project accounting for itself. The blocked rows to name are **D-U1, D-U4, D-U7** (Anthropic key) and **D-U2, D-U5** (Elliot).
