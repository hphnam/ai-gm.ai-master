...
| **RQ4** | *Does a split-conformal interval hold its nominal coverage at every venue in the estate, and where it does not, which property of the data accounts for the departure?* **AMENDED 2026-08-09: "shortfall" → "departure".** A shortfall is a downward miss, and the served band misses nominal at **all three** venues in **both** directions, so the original word presupposed an answer the evidence contradicts — the same defect §6's C3 amendment struck when it removed *"the one venue that under-covers"*, surviving one section away. `introduction.tex`:168 and `discussion.tex`:86 both read "departure", so R8 traceability is served rather than broken. |
...
than *why it fails*, even though the answer is that it misses nominal at **all
three** venues in opposite directions — over-covering at Two River Taps,
under-covering at the Beer Hall on the marginal limb, and failing in the
**unsafe** direction at Ellel on the traded limb, where coverage is $0.692$.
**CORRECTED 2026-08-09**: this note read *"the answer is that it fails at one
venue"*, which is the presupposition the paragraph exists to warn against,
asserted by the warning itself. It is also what the amended RQ4 string above
removes, and what §6's C3 amendment removed. **Three sites, one claim, repaired
in three separate passes** — the ledger rule against restating a fact in more
than one store, failing inside the file that fixes the strings.
...
still live and is carried forward to 8D.**~~ **CLOSED 2026-08-09**, at the root and at eight
downstream sites. The differences reproduce from the artefacts as 0.001143 / 0.001206 / 0.001570.
...
| **C2** | RQ3 | *A controlled test of weather and of cross-series pooling at venue-total aggregation, separating the covariate arms by the lead at which each is actually available.* | **Full. AMENDED 2026-08-09 against the evidence; the previous cell read "Both arms return nulls; state the null as the finding" and that is false for pooling.** Say **neither arm buys accuracy**, and state each limb as it measures. **Weather** is a set-level null (all five arms retained at every venue) carrying **one pairwise exception that must be stated with it**: the Beer Hall no-weather against horizon-matched contrast excludes zero at $+0.0163$, $[0.0004, 0.0337]$, a marginal detection the set does not sustain. **Pooling is not a null everywhere**: `tab:group` retains **`{U}` alone at Two River Taps**, so both grouped arms are *eliminated*, and the paired intervals exclude zero at the Beer Hall and Two River Taps; only Ellel spans zero. It is **no improvement, with a detected small loss at the two data-rich venues**. **Gap-filling claim, also amended:** the weather limb fills the missing controlled hospitality test at this aggregation; **the pooling limb fills no gap**, this estate sitting below the scale at which the pooling literature poses its claim. **And the validity cross-reference is one-sided:** `discussion.tex` §5.3 says in terms *"Neither measurement reaches the weather and pooling nulls"*, so 5.3 may be cited for the model-comparison arms **only**, together with its open item that these result files carry no environment identity. Writing this limb as a clean matched pair of nulls would be the mirror image of the over-claiming C2 exists to correct. |
| **C3** | RQ4 | *A per-venue calibration audit of a split-conformal interval, identifying the exchangeability violation responsible for the venue that under-covers in the unsafe direction and reproducing the measured coverage from it.* | **Full and the strongest. AMENDED 2026-08-09; the previous cell read "Reproduces measured coverage to a thousandth at all three venues" and that is refuted.** `discussion_rewrite_critique.md` **B13** measures the implied-versus-measured agreement at **0.00114 / 0.00121 / 0.00157**, so a thousandth is **not met at any venue**. Say the rank decomposition **predicts each venue's coverage in sign and rough size**, and say in the same breath what B13 established as the stronger point: it **decomposes the same indicators rather than measuring them independently**, so it locates the departure without narrowing how large it is. ~~**The false phrase is still live at `results.tex`:650 and `discussion.tex`:93** and is an open 8D repair, §F.~~ **CLOSED 2026-08-09.** Repaired at nine sites, and at the ROOT first: `log/72_DU6_exchangeability_result.md`:69 asserted it, and that file is a `*result*.md`, so it is T1's own terminal node and every trace check passed on it. See `log/72` §7. Also amended: *"the one venue that under-covers"* was loose — the band misses nominal at **all three** venues in opposite directions, and the one failing in the **unsafe** direction is **Ellel at $0.692$ on trading days**, which is invisible in the marginals. |
| **C4** | RQ5 | *An evaluation of deviation detection under an asymmetric cost, on a measure committed to before the results were seen, with the injection design's contribution to the score measured rather than assumed.* | **Full on the measurement, qualified on the operating point.** The cost sweep selects no operating point, and the reason is itself a finding: **124 misses against 75 spurious** inverts the failure mode the literature guards against. **CORRECTED 2026-08-09: this cell read "8 false alarms against 124 misses" and the 8 was withdrawn.** The 8 was the fatigue count — items surfaced on **un-injected** windows across three venues, a weekly deployment-rate upper bound — summed against a miss count from the **injection corpus**, which is a confusion matrix over two different populations. The corpus figure is 75 (`log/PRJ93_Agent_Eval_Report.md` S6, N=644). The withdrawal is recorded at `eval/agent_eval.py`'s `cost_curve` docstring, which names the exact substitution. Note the ratio still inverts, and by more: 124:75 rather than 124:8 understates nothing about the direction, only the margin. |
