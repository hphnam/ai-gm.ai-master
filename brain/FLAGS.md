...
- **FLAG-EVAL-HARNESS-UNGUARDED (OPEN, and this one touches a printed number).**
  ...
  **AMENDED S14, verified at source rather than from the report.** Both halves of
  the mechanism above were re-read in the code and both hold.
  `eval/agent_eval.py:318-330`: `fatigue_metrics` calls `inject.holdout(stream)`
  directly on `inject.base_stream(v)` and never calls `_usable_folds`.
  `_usable_folds` at `agent_eval.py:418-425` is the function that carries the
  filter, and it is explicit about why: *"For a closed venue, only PRE-closure
  folds, a post-closure window is all structural zeros and the closure dominates
  (respect closure, GA1)."* The two functions therefore disagree about the same
  venue in the same module, and the one WITHOUT the filter is the one that feeds
  the printed number. `_signals_from_stream`'s docstring at `agent_eval.py:57-59`
  reads *"exactly as `briefing.collect` does from the store, but over the
  perturbed stream"*, and `briefing.collect` reaches `_collect_deviation`, which
  computes `closed`/`aend` at `briefing.py:133` and applies G5c at `:156-158`.
  **The parity the docstring asserts is not delivered, and the missing half is the
  closure half.**
  **One thing bounds the damage and it was already in the code.**
  `fatigue_metrics`' own docstring says *"On real data these alarms may be genuine,
  so this is an honest UPPER BOUND on the weekly false-alarm rate, not a count of
  known-false alerts."* The generator never claimed the 8 were false alarms, and
  `results.tex:911` prints it as an upper bound. The defect widens a bound that is
  already declared as one. That is why the disposition stays OPEN and unfixed
  rather than becoming a correction.
  **Wrong selector, not only a missing guard.** Filed here as a selector defect:
  the guard exists in the module and the caller reaches past it. A fix that adds a
  third closure filter to `_signals_from_stream` without redirecting
  `fatigue_metrics` at `_usable_folds` would leave the window post-closure by
  construction and repair nothing.

- **FLAG-CLOSURE-ARTEFACT-CONTRADICTIONS (OPEN, uncited, evidence-chain only).**
  Two committed artefacts state the closure behaviour and then contradict it in
  their own body. Neither is cited by any `.tex` file (`grep` over `chapters/`,
  `appendix/` and `main.tex` for `july2026_brain_result` and `deviation_eval`
  returns nothing), so no reported number moves. Both sit in the evidence chain a
  reader of the artefacts would walk, which is why they are recorded rather than
  left.
  **(1) `sim/july2026_brain_result.json`.** Its `trt_closed_note` reads *"Two
  River Taps is_closed; the briefing marks its closure dormant (no routine
  deviation items) and the persisted chain labels it continuing, not a daily new
  alarm (contrast June's 6 downward deviations)."* The same file's `deviation`
  block reads `two_river_taps: {"n_deviation": 1, "flagged": [{"date":
  "2026-07-03", "direction": "down", "z": -1.26, ...}]}`. The 2026-07-03 fire is
  number 14 of the fourteen in FLAG-DEV-CLOSURE-UNGUARDED. **The note describes
  the briefing path, which is guarded and correct; the block reports the raw
  primitive, which is not. The file does not say which surface each belongs to,
  so as written it asserts and denies the same thing.**
  **(2) `eval/deviation_eval.md`.** Its Two River Taps row, line 10, carries date
  `2026-07-05`, class `normal`, `z = -0.60`, `actual = 0.0`, band `-203.42` to
  `802.39`, with the direction and attribution cells empty. The footnote at line 12
  reads *"Trading days only (the shared stream excludes structural-zero days), so
  Ellel fires only on genuine booking days (FLAG-PD1)."*
  Two River Taps' last active day is 2026-05-08,
  so a 2026-07-05 row with `actual = 0.0` is exactly a structural-zero day and the
  footnote is false for it. **Note the row is classified `normal`, not
  `deviation`: at `z = -0.60` it is inside the `DEV_BAND_K = 1.0` band, so this is
  a false statement about the stream's POPULATION and not a false fire.** It is the
  same root cause as FLAG-PD1's narrowing: the trading-day test is a property of
  the venue's history, so a post-closure day passes it.
  **Fix, after submission and with the primitive's guard:** both artefacts are
  regenerated by the re-run already listed under FLAG-DEV-CLOSURE-UNGUARDED, so
  neither needs separate work. The `trt_closed_note` wording needs a surface named
  either way, since it would still be describing the briefing while the block
  reports the primitive.
  **Evidence:** report 85 section 1.2 item 11; both artefacts re-read at source in
  S14.
