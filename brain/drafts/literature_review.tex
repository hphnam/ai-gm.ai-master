...
data at all. A three-venue estate with one year of trade sits well inside the
sample-size regime both models were built for, by more than an order of
magnitude, and Section~\ref{sec:ladder} gives the figures. Only one of the two
was evaluated here. TabPFN-TS was entered as a rung of this study's ladder and
withdrawn before it scored, for a reason with nothing to do with forecasting:
its weights are released through a vendor account, and the library's default
entry point sends the series to a hosted service, which this estate's data may
not use. Nothing in this dissertation licenses a statement in either direction
about how it would have performed. This dissertation assumes, without
demonstrating, that the covariates most worth conditioning on in hospitality are
known-future calendar variables rather than weather; no source surveyed here
separates the two channels in a way that would establish the split.
...
conformity scores to be almost surely distinct. That condition fails on the data
this work concerns, and it fails more widely than a first reading suggests. A
series of mostly zeros produces ties as a matter of course. So does any venue
with a closing day: when the doors are shut the actual and the forecast are both
zero, the absolute residual is exactly zero, and the score distribution carries
an atom there. Structural closure rather than sparsity is what breaks the
condition, so the upper bound is unavailable wherever a trading calendar has
holes in it, which on this estate is everywhere.
...
on absolute errors optimise for the median, and on intermittent series, they
write, such measures consider constant zeros the best prediction. Their statement
carries no condition. The threshold at which it becomes exactly true is derived
here rather than taken from them: the median of a series is zero precisely when
more than half its observations are zero, so the minimising constant is zero only
once zero periods outnumber trading periods. That is $p > 2$, stronger than the
$p > 4/3$ which classifies a series as intermittent above, and a venue trading
one or two days a week sits well inside it. The same paper notes a companion
hazard for benchmark-scaled measures, that a naive benchmark giving exact zeros
on zero actual values is problematic for the scale those measures divide by.
Describing that as a deflation of the denominator is this chapter's
characterisation; the source names the problem without assigning it a direction.
Both failure modes
...
yield the lowest cost when lumpiness is high. The remedy that finding points to
carries a precondition easily read past. The cost being minimised is the cost of
an inventory system, priced in ordering, holding and shortage rates against a
stock position, so it is defined only where the forecast is itself a
replenishment quantity; where the quantity forecast is not held, ordered or
backordered, there is no unit for those rates to price. A second and narrower
part of the same study transfers without that precondition. Working inside the
error family, \citeauthor{chatfield_all-zero_2007} had to modify the percentage
measure because it cannot divide by a demand of zero, and to drop a geometric
measure recommended elsewhere for intermittent demand because it breaks down on
forecast errors of zero. A study whose own denominator-bearing measures degrade
on zero-heavy demand is direct support for dropping the denominator where the
zeros are thickest, which is a smaller claim than changing the objective and a
better supported one. Finally the point forecast may be
...
estate, and this dissertation adopts it. Headline accuracy figures are root mean
squared scaled error, with an unscaled root mean squared error at the venue
where no scale basis proved defensible, and the mean-absolute-scaled measure is
carried throughout as a labelled secondary so that a conclusion resting on only
one of the two is visible as such. Adopting the measure does not dissolve the
tension so much as relocate it. The served foundation model returns a median
under a mean's name, as Section~\ref{sec:ruler-functional} documents, so the
measure this estate is now scored on elicits a functional the served forecaster
cannot produce, and the remedy was declined on the grounds that it would change
the served model after the evaluation was frozen. That remains a limitation of
the work rather than a point in its favour.
...
estimate improves. One design consequence follows, and it is this chapter's
inference rather than anything the source argues.
\citeauthor{sun_conformal_2025} treat the regime as latent throughout, since a
latent regime is the case their method is built for, and they say nothing about
the alternative. But if the misclassification rate is the currency the bound is
paid in, a regime variable that is simply \emph{recorded} costs nothing, and
whether a venue is trading is recorded in the till rather than hidden in the
residuals. The step is one line and the reader can check it. What it does not do
is carry any guarantee the paper proves about its own online procedure across to
the static bands used here, which are a different construction and earn their
coverage elsewhere. A detector can establish \emph{that} trade has broken from
its rhythm. It cannot decide whether the break is worth a manager's attention.
...
carried forward to the discussion of the work as a whole. The measure this
chapter argues for is the measure the work now reports, so what survives on that
front sits one step further in: the served foundation model returns a median
under a mean's name, which leaves the squared-scaled headline eliciting a
functional the forecaster in production cannot emit, and the remedy for that was
declined rather than missed (Section~\ref{sec:rw-ruler}). The expected
calibration error that Section~\ref{sec:rw-evaluation} identifies as the
instrument capable of carrying a coverage guarantee through to the agent's
output is not computed anywhere in this work. And the inter-demand interval
estimated for each venue carries sampling uncertainty
Section~\ref{sec:rw-rhythm} leaves unquantified. Against the
