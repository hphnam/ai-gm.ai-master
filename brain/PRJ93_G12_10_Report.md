...
- **Material finding.** Removing the leak dropped Ellel's rolling `rung3_gbm` from a leakage-inflated **0.533** (G12.9c) to an honest **0.813**, and `rung3_global_gbm` to 0.936. The earlier GBM "win" on Ellel was substantially a leak artifact. See **G12.10b** for the consequent milestone outcome.
...
- **Ladder rerun (all venues, real chronos), MASE against the calendar-only number it replaces:**
  ...
  | ellel | 0.570 | 0.591 (worse) | `rung1_robust_dow` (0.572) | **FAIL** |

  **Honest outcome, gate-decided:** the fuller factor set **helps the anchor** (BH 0.779 → 0.745), is roughly neutral for TRT, and **adds noise on the sparse Ellel series** (0.570 → 0.591). Combined with the a2 leak fix (which removed the GBM's inflated Ellel win), **no Ellel rung beats the robust-DOW baseline (0.572)**, so Ellel's milestone now FAILS and Ellel is served by Rung 1. This is a valid gate outcome per the spec ("a worse exo MASE from the fuller factor set is a valid gate outcome to report"), and it means the original Rung-1 hypothesis for Ellel is now substantially vindicated **empirically** (not by an assumed cap; Ellel remains uncapped and every rung competes, the gate simply adopts none above DOW). The BH milestone (the run's exit gate) passes, so the ladder run exits 0.
...
1. **Ellel milestone now FAILS (nothing beats robust DOW), a stronger reversal than the spec anticipated.** The spec framed G12.10b around "more covariates can help or add noise… the gate decides adoption as always" and G12.10a2 around fixing a leak. The combined effect is sharper than either alone: with the leak removed AND the fuller (noisier) covariate set, the best Ellel rung (`rung4_chronos2_exo` 0.591, `rung4_chronos2` 0.581) no longer beats robust DOW (0.572), so Ellel adopts Rung 1. This is explicitly a valid gate outcome, but it partially walks back G12.9c's headline ("a higher rung IS adoptable on Ellel"), that conclusion was substantially a leak artifact. Recorded here and in FLAGS/decision-log terms as the honest post-leak finding, not hidden.
