...
| **THE REDUCTION PLAN IS PREPARED AND NOT EXECUTED, AND IT DOES NOT REACH 15,000** | `ledger/reduction_plan.md`, 2026-08-09, item 2 of the 8C-8 brief. PROTECT / COMPRESS / DEMOTE per paragraph across all six chapters, each item with its criterion or its reason, its measured marker cost, and what the body retains. **Ceiling total 22,031 texcount; at the 70 per cent realisation the S-4 precedent argues for, 24,051.** Against a 15,000 target that is **over by 7,031 at best**, and over the 18,000 acceptability bound by 4,031. **Stated rather than engineered downward.** The three populations large enough to close a 7,000 gap are Chapter 4's protected core, Chapter 2 below its funnel, and Chapter 6's 6.2 with Chapter 5's 5.1 — which break, respectively, every headline verdict's support in the body, R7, and R8 plus the exact C2/RQ2 omission the directional-compression rule exists to prevent. **Largest single reduction available: Chapter 3 at 5,686 → ~3,300**, because Appendix B exists and `ds-writing` §8's replicability test is satisfied by the document rather than by the chapter. **Highest single item: `discussion.tex` 5.3 ¶2 at 394**, which restates Chapter 4's paired standard errors. **Phuong rules section by section** |
| **A lever the marker instrument cannot see: captions are inside the counted population** | 872 caption words, 771 of them in Results across 14 floats averaging 55 words of caption body. Compressing a caption or demoting a float to an appendix **reduces the governing number and shows as ZERO in `wordcount.py`**, which strips floats before counting. Any plan costed only in marker words is blind to it |
| **Appendix C is an empty shell and `completenesscheck` cannot see it** | `appendix/robustness.tex` carries **8 words of body prose**. It clears the 40-word content floor because the floor tests `count(raw)`, which includes caption and table text, and its two float captions total 177. Its own header reads *"Prose for this appendix is composed by 8C-7"* — a TODO no instrument reads. **This is the defect class the tool was built for, sitting in the tool's own blind spot: a file consisting entirely of floats passes a prose floor designed to catch a section nobody wrote.** Reported, not fixed — fixing it mid-plan would move the pre-flight baseline. **It is also the prepared destination for Chapter 3's 1,466 words of DEMOTE** |
| **`methodology.tex`:382–384 promises content Appendix C does not contain** | It states the paired loss differential's variance *"is measured rather than assumed, and Appendix~\ref{app:robustness} reports it with a block-length sweep."* Appendix C has **no paired-variance material and no block-length sweep**. The `\ref` resolves to the appendix, so `latexcheck` is silent and no instrument here reads a cross-reference for whether its target says what the citing sentence claims. Found while chasing `numbers_audit` row 62 |
...
| **`venueordercheck`: only ONE of five sits in text the reduction pass rewrites** | `results.tex`:782 (UNANCHORED, native intervals) is DEMOTE material and goes with it. `results.tex`:513 is partly inside a compress target. **The other three need their own repair: `results.tex`:352 and :406 are inside TABLE FLOATS in protected 4.3 material, and `discussion.tex`:29 is in 5.1, protected under R8.** Note the two float cases edit caption and table text, which is **inside** the governing count — naming the venues inline is the correct fix and costs ~20 words |
