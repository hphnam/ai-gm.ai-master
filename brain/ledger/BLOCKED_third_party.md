...
| Chapters composed to the approved tree | **4 of 6** — Chapter 2 (8C-1), Chapter 3 (8C-2), Chapter 4 (8C-3), Chapter 5 (8C-4). *Instrument: `git log` on the clone* |
| Chapters composed **and pushed** | **4 of 6.** `origin/main` is **`fe7bd9a`** (pushed manually 2026-08-08 after the protected-branch guard refused the agent's push a second time — the guard is now a standing feature of this workflow, not an incident). *Instrument: `git ls-remote --heads origin`; never assume from a push's exit code.* **8C-4 supplies the counter-example that makes this row load-bearing:** the first manual push of 8C-4 reported success and moved nothing, because the work was still uncommitted — `git ls-remote` returned `f903214` while `chapters/discussion.tex` was untracked. *"I pushed"* and *"the remote holds it"* are different claims |
| Chapter files live on Overleaf with prose in them | **5** — Chapters 2–5 plus `conclusion.tex`, the last still composed to a *pre-tree* five-chapter shape (`sec:conclusion-divergences` and `sec:conclusion-limitations` excised by 8C-4 under A5/§3.5 and relocated into 5.2/5.4). `introduction.tex` is an empty template stub |
| Measured word floors — HISTORY, no longer held against per-section budgets (S-3) | Ch 2 **4,938** (raw 4,948) · Ch 3 **5,526** (raw 5,618) · Ch 4 **6,247** (raw 6,274) · Ch 5 **4,646** (raw 4,653). The budgets they were once measured against are this project's own allocation under A10, not a rubric requirement; only HC1's 20,000 total is mechanical. **Ch 4's 6,247 was measured on the composed version and the queue's items 1–3 change that text**, so re-measure before S-4 decides custody on space. *Instrument: `brain/scripts/wordcount.py <file> <key>`* |
...
| **TIER 2 IS UNAVAILABLE — the local compile cannot be run** | **NEW 2026-08-08, and it invalidates an instruction this file gives.** No TeX binary is reachable: `command -v latexmk pdflatex xelatex tex` returns nothing, `/Library/TeX` does not exist, and there is no `/usr/local/texlive`. Confirmed both inside and outside the tool sandbox, so it is absence and not a permissions artefact. **TeX was working earlier the same day** — 8C-3 compiled at 115 pages and 8C-4's fresh-clone run at 125 — so the installation was removed or lived on a volume no longer mounted. **Consequence:** the compile row above says *"Do not copy these numbers forward — re-run it"*, and that instruction is currently impossible to follow. `PRJ93_RULES.md`'s three-tier boundary assigns everything downstream of line breaking to tier 2; with tier 2 gone the estate has **tier 1 and tier 3 only**, and no pre-flight stands between a push and Overleaf. **Do not push a `.tex` change until TeX is reinstalled**, or accept explicitly that the push is unpre-flighted and say so. *Instrument: `command -v latexmk`* |
| `abstract.tex` | **UNWRITTEN — template boilerplate, in bold, live on `origin/main`.** Not a formatting defect: the file still carries the issued placeholder and a `bit.ly` link. It survived every push and two fresh-clone compiles because **a compile verifies that a document builds, not that a section was written**, and no instrument in this project asks the latter. `ds-writing` §5 rates the abstract the most important section in the dissertation. *Instrument: open the file* |
| T8, source-claim verification | **DISCHARGED FOR CHAPTER 5, OPEN FOR CHAPTER 4.** `ledger/source_claim_verification.md` holds the run of 2026-08-08 against the `Dissertation` notebook (118 sources), covering all 21 keys / 28 citation commands in Chapter 5: **16 supported, 2 refuted, 2 unreachable** because the source is not in the notebook. The two refutations are **H12-1** (the Mondrian construction attributed to `barber_conformal_2023`, whose sole related-work mention of it disclaims the connection and credits Vovk, Gammerman & Shafer 2005 — **absent from both `.bib` files**; originates at `methodology.tex`:442, inherited at `discussion.tex`:214) and **H12-2** (`stocker_gentle_2025` over-attributed a group-conditional guarantee at `discussion.tex`:215, where `methodology.tex`:443 is careful — a Chapter 5 compression error). Chapter 2's two uses of `barber_conformal_2023` are **correct** and must not be touched. *Instrument: that file; it quotes every source sentence it relies on* |
| Role-critique findings against Chapters 4 and 5 | **OPEN, REPORTED, NOT REPAIRED.** `ledger/role_audit_ch4_ch5.md` holds the six critiques; `ledger/role_audit_synthesis.md` holds the ordered repair list and the gate split. **No chapter text was changed.** Ordering is owned by the sequencing note below — do not restate it here. *Instrument: those two files* |
