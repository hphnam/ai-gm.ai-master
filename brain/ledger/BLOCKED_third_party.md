...
| Measured word floors, all four PROVISIONAL pending reallocation | Ch 2 **4,938**/4,000 (raw 4,948) · Ch 3 **5,526**/4,200 (raw 5,618) · Ch 4 **6,247**/5,200 (raw 6,274) · Ch 5 **4,646**/2,400 (raw 4,653). *Instrument: `brain/scripts/wordcount.py <file> <key>`; values held here because a floor costs a run and is under active decision* |
...
| Role-critique findings against Chapters 4 and 5 | **OPEN, REPORTED, NOT REPAIRED.** `ledger/role_audit_ch4_ch5.md` holds the run of 2026-08-08 — SKILL.md §3's three roles plus the §4 T1–T14 gate, six independent calls. Chapter 4 had never had a §3-role pass; Chapter 5's is a replication of 8C-4's. **No chapter text was changed.** Two items carry mandated ordering: §5's contradiction signal (X1) resolves before any other revision, then T1. *Instrument: that file; do not re-derive the counts here* |
