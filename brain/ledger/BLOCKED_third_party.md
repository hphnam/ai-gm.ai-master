...
| Document builds, and every checker it feeds — **CURRENT ROW, 2026-08-12** | **Measured on the working clone at `762a028`** (`--shell-escape` supplied, `svg.sty` stub via `TEXINPUTS`, build directory in the scratchpad so `git status` stayed clean): **144 pages, 0 errors, 0 undefined references, 0 undefined citations, 0 floats lost, 0 refused `\write18`**; **3 overfull, all under 3 pt** — the 88.49 pt Hansen box is **gone**, it was the `\_eprint` bibliography artefact and not a typesetting problem. `\write18` **= `texcount` = 19,989**, margin 11. `completenesscheck` PASS (26 files) · `figurecheck` PASS (**26** sources) · `venueordercheck` PASS (26 files, was FAIL 3) · `formatcheck` PASS with **one** ruled spill (was two) and **0** unreferenced floats (was 4) · `commentsweep` 26 files, 2 reviewed grammatical joins. **Every population is stated because a checker's population is part of its result** — the row above ran `figurecheck` over 20 sources and `completenesscheck` over 28, and a third swallowed clause was living in the files a 13-file sweep did not open. *Instrument: the command at the head of this section, plus `--accept ledger/format_accepted.txt --aux` for the float section, which the previous run never passed.* **Tier 2 only:** TeX Live 2026 locally, which is not Overleaf's until T3-1 closes |
| ~~Document builds, and every figure a build reports~~ **SUPERSEDED 2026-08-12 by the row above** | **VERIFIED ON A FRESH CLONE OF THE LOCAL COMMIT `45e4090`** (`main-words.sum` confirmed ABSENT; `svg.sty` stub supplied via `TEXINPUTS` from scratch, so the clone stayed clean — `git status` empty after the build): **146 pages, 0 errors, 0 undefined references, 0 undefined citations, 0 floats lost**, **3 overfull** (88.49 pt Hansen `note` field, 2.54 pt and 1.97 pt in centred TikZ floats), **7 underfull**. **This is a clone of a LOCAL commit, NOT of the pushed state** — `origin/main` is `ec334a64` and three formatting commits await a push. Also run on that clone: `completenesscheck` PASS (28 files), `figurecheck` PASS (20 sources), `venueordercheck` FAIL 4 — **pre-existing, verified identical on `ec334a64`**. Ink outside the text block **6 pages of 144 → 2 of 146** (`ledger/formatting_pass_2026-08-11.md`). *Instrument: the command at the head of this section. Do not copy these numbers forward — re-run it.* **Tier 2 only:** TeX Live 2026 locally, which is not Overleaf's until T3-1 closes |
...
| **~~FINAL RULING ON LENGTH — 19,000 HARD~~ — SUPERSEDED 2026-08-11. THE CAP IS 20,000** | **This row no longer states the ruling; `knowledge/00_marking_criteria.md` §1.1 does, and it is the store that owns rulings.** 19,000 was Phuong's margin against forecast error and was withdrawn as unreachable, not as wrong — see the §F row below, which recorded the withdrawal on 2026-08-10 while this row went on asserting the opposite. **Two rows of one file disagreeing about a hard constraint is the store-ownership failure at its smallest radius**, and it survived here because a withdrawal was written where the work was tracked rather than where the claim was made. What still holds from the 2026-08-09 ruling: **15,000 is off the table**; ~22,000-with-a-justification must not be re-offered; and under a ceiling **the lever is DEMOTION**, whose limit is not budget but whether the body still discharges its criteria. |
