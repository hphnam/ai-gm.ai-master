...
function MessageBubble({
  ...
  return (
    <li className={cn('group flex flex-col gap-0.5', fromMe ? 'items-end' : 'items-start')}>
      {showAuthor && author ? (
        <span className="px-2 text-[10px] text-foreground/55">{author.name ?? author.email}</span>
      ) : null}
      <div className={cn('flex items-center gap-1', fromMe ? 'flex-row' : 'flex-row-reverse')}>
        {/* Hover-revealed ⋮ on the OUTSIDE of the bubble (i.e. left of */}
        {/* yours, right of theirs) so it never overlaps the text. */}
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button
              type="button"
        ...
        <div
          className={cn(
            'relative max-w-[78%] rounded-2xl px-3 py-1.5 text-sm leading-snug shadow-sm',
            fromMe
      ...
      <div
        className={cn(
          'flex items-center gap-1.5 px-2 text-[10px] text-foreground/45',
      ...
      >
        ...
        {fromMe && message.kind === 'note' ? (
          <span title={message.status === 'read' ? 'Read' : 'Delivered'}>
            {message.status === 'read' ? '✓✓' : '✓'}
          </span>
        ) : null}
      </div>
