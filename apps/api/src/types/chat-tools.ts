...
export const TOOL_DEFINITIONS: ReadonlyArray<{
...
}> = [
  {
    name: 'find_knowledge',
    description:
      'Hybrid retrieval (vector + BM25 lexical, fused via reciprocal-rank fusion) across knowledge items (SOPs, troubleshooting, Q&As), individual checklist steps, venue contacts, suppliers, and venue profiles. Use for any question whose answer lives in operational knowledge — procedures, troubleshooting, "who do I call", "what\'s step 3 of...", "where\'s the fire escape", etc. Hits include `entityType` so you know whether you got a doc, a checklist step, a contact, etc. Returns ok:false reason:no-data if nothing matches semantically OR lexically. SOURCE PRIORITY: do NOT use this for LIVE business data — sales totals, current stock counts, recent orders, payment / tender mix, refunds, labor cost, who\'s on shift, today\'s takings, best-sellers. Those have dedicated pos_* tools (Square today, more integrations later) and are AUTHORITATIVE over the KB — the integration returns live numbers; the KB at best has an uploaded snapshot. Even if the user has uploaded a doc on those topics, prefer the pos_* tool unless the user explicitly asks for the uploaded version.',
    input_schema: {
      type: 'object',
      properties: {
        query: {
